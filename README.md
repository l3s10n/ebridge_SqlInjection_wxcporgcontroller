...
wxcporgcontroller存在sql注入漏洞，在登录泛微云桥之后，可以借助该漏洞执行sql注入语句。
...
wxcporgcontroller.exportOrg函数调用了wxCpOrgInfoService.exportOrg函数，该函数关键代码如下：
...
public File exportOrg(String syscorpid, String depts, String users, String fields, String syncond) throws Exception {
    ...
    if (!StrKit.notBlank(depts) && fields.indexOf("efield_dept") == -1) {
        if (StrKit.notBlank(users)) {
            sql.append(" and u.id in (" + ToolString.buildSqlInClause(users.replaceAll("\\|", ",")) + ")");
        }
...
wxcporgcontroller.exportOrg调用wxCpOrgInfoService.exportOrg函数的方式是这样的：
...
public void exportOrg() {
    try {
        ...
        if (StrKit.isBlank(syscorpid)) {
            ...
            this.renderFile(this.wxCpOrgInfoService.exportOrg(syscorpid, depts, users, fields, syncond));
        }
